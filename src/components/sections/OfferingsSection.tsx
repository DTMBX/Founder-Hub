/**
 * Offerings Section - Refactored
 * 
 * Display offerings using the new OfferingCard component.
 * Features category filtering, animations, and detail dialogs.
 */
...
import { OfferingCard, OfferingGrid } from '@/components/offerings/OfferingCard'
import {
  Globe,
  FileText,
  Code,
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        >
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-4">
            {tradeMode ? 'Trade & Barter' : 'Services'}
          </h2>
        ...
        {enabledOfferings.length === 0 ? (
          ...
          <AnimatePresence mode="popLayout">
            <OfferingGrid
              offerings={enabledOfferings}
              columns={3}
              onViewDetails={setSelectedOffering}
              contactEmail={contactConfig.emailTo}
            />
          </AnimatePresence>
        )}
        ...
        >
          <GlassCard intensity="low" className="inline-block px-8 py-6">
            ...
            <GlassButton
              variant="glass"
              onClick={() => window.location.href = `mailto:${contactConfig.emailTo}?subject=Custom Request`}
            >
                    ...
                    <div className={cn(
                      ...
                      {o.priceTiers.map((tier) => (
                        <GlassCard
                          key={tier.id}
                          intensity="low"
                          className="p-5 flex flex-col hover:border-accent/40 transition-colors"
                        >
                  ...
                  <div className="pt-6 border-t border-border/40 flex flex-col sm:flex-row gap-3">
                    {o.pricingType === 'contact' ? (
                      <GlassButton
                        ...
                        onClick={() => window.location.href = `mailto:${contactConfig.emailTo}?subject=${encodeURIComponent(o.title)}`}
                      >
