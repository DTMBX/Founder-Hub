...
import { runHonorBarLayoutGuard, verifyZIndexOrdering } from '@/lib/honor-bar-guard'
...
export interface HonorFlagBarProps {
  enabled?: boolean
...
export default function HonorFlagBar({
  ...
  // Publish bar height as CSS custom property on :root for global layout coordination
  useEffect(() => {
    document.documentElement.style.setProperty('--honor-bar-height', `${barHeight}px`)
    return () => {
      document.documentElement.style.removeProperty('--honor-bar-height')
    }
  }, [barHeight])

  // Dev-only: verify no ancestor clips the bar and z-index ordering is correct
  useEffect(() => {
    runHonorBarLayoutGuard(barRef.current)
    verifyZIndexOrdering(barRef.current)
  }, [])
